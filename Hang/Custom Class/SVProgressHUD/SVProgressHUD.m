...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDForegroundColor =[UIColor colorWithWhite:0.1 alpha:0.9];
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    CGFloat keyboardHeightNew = 0.0f;
    ...
    if(notification) {
        ...
        if(notification.name == UIKeyboardWillShowNotification || notification.name == UIKeyboardDidShowNotification) {
            if(ignoreOrientation || UIInterfaceOrientationIsPortrait(orientation))
                keyboardHeightNew = CGRectGetHeight(keyboardFrame);
            else
                keyboardHeightNew = CGRectGetWidth(keyboardFrame);
        }
    } else {
        keyboardHeightNew = self.visibleKeyboardHeight;
    }
    ...
    if(keyboardHeightNew > 0)
        activeHeight += CGRectGetHeight(statusBarFrame)*2;
    
    activeHeight -= keyboardHeightNew;
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + 44 + self.offsetFromCenter.vertical);
}
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.06 , 3.0);
}
...
- (UIImageView *)imageView {
    if (!_imageView)
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 48.0f, 48.0f)];
